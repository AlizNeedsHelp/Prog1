...
private:
    ...
    // funkciok mukodese, vagy amikor "megnyomódnak"
    ...
    smenu_button(Point(x_max()-80,60),80,20,"style menu",cb_smenu)
{
    ...
    attach(color_menu);
    color_menu.hide();
    attach(cmenu_button); //rámegyünk a color menüre, ez megjelenic, color menu "gomb" eltünik
    //kiválasztunk egy szint és megjelenik
...
void Lines_window::next()
{
    ...
    // kiirja az uj megadott koordinatakat , ujrarajzol
    ostringstream ss;
    ss << '(' << x << ',' << y << ')';
    xy_out.put(ss.str());
